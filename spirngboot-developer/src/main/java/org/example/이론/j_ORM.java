...
public class j_ORM {
    ...
    // cf) userPassword (lowerCamelCase) - 클래스

    // 1) ORM의 특징
    // - 객체와 테이블이 매핑: 1:!로 연결하여 데이터를 매핑
...
//    }
    ...
    // : 기본 키 자동 생성 전략 설정
    // +) 다양한 옵션이 있음
    //      - strategy 옵션: GenerationType.IDENTITY(auto_increment)
    ...
    // +) 다양한 옵션이 있음
    //      - name옵션: 열 이름 지정
    //      - nullable옵션: 열이 null값을 허용 할지 여부를 설정 (기본값 true)
